resource "aws_instance" "payment" {
  ...
  tags = merge(
    {
        Name = "${local.common_name}-payment-${var.app_version}-${var.environment}"
    },
    local.common_tags
  )
}
...
resource "aws_ec2_instance_state" "payment" {
  ...
  depends_on = [ terraform_data.payment ] # we are saying stop only after creating and configuring instance, so 1 and 2 should manadetory to run 3
}
...
resource "aws_ami_from_instance" "payment" {
  ...
  depends_on = [ aws_ec2_instance_state.payment ] # this depends upon 3
  tags = merge(
    {
        Name = "${local.common_name}-payment-${var.app_version}-${aws_instance.payment.id}"
    },
...
# 5 launch template for payment
resource "aws_launch_template" "payment" {
  name = "${local.common_name}-payment-launch-template"
  image_id = aws_ami_from_instance.payment.id # we are taking from current instance
...
resource "aws_lb_target_group" "payment" {
  name     = "${local.common_name}-payment-tg"
  port     = 8080
  protocol = "HTTP"
  vpc_id   = local.vpc_id
...
resource "aws_autoscaling_group" "payment" {
  name                      = "${local.common_name}-payment-asg"
  max_size                  = 10
  min_size                  = 2
...
resource "aws_autoscaling_policy" "payment"{
  name                   = "${local.common_name}-payment-scaling-policy"
