resource "aws_instance" "cart" {
  ...
  tags = merge(
    {
        Name = "${local.common_name}-cart-${var.app_version}-${var.environment}"
    },
    local.common_tags
  )
}
...
resource "aws_ec2_instance_state" "cart" {
  ...
  depends_on = [ terraform_data.cart ] # we are saying stop only after creating and configuring instance, so 1 and 2 should manadetory to run 3
}
...
resource "aws_ami_from_instance" "cart" {
  ...
  depends_on = [ aws_ec2_instance_state.cart ] # this depends upon 3
  tags = merge(
    {
        Name = "${local.common_name}-cart-${var.app_version}-${aws_instance.cart.id}"
    },
...
# 5 launch template for cart
resource "aws_launch_template" "cart" {
  name = "${local.common_name}-cart-launch-template"
  image_id = aws_ami_from_instance.cart.id # we are taking from current instance
...
resource "aws_lb_target_group" "cart" {
  name     = "${local.common_name}-cart-tg"
  port     = 8080
  protocol = "HTTP"
  vpc_id   = local.vpc_id
...
resource "aws_autoscaling_group" "cart" {
  name                      = "${local.common_name}-cart-asg"
  max_size                  = 10
  min_size                  = 2
...
resource "aws_autoscaling_policy" "cart"{
  name                   = "${local.common_name}-cart-scaling-policy"
